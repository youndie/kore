...
| `kore-booblik/` | **not built** — flush-then-close for booblik, and [B-36](../backlog/B-36-booblik-adapter-targets.md) decided its shape: **multiplatform with a per-platform actual**, one over each of booblik's two client APIs. D5's "JVM only" was withdrawn when `booblik-native` turned up on Central; the module itself arrives with [B-15](../backlog/B-15-booblik-adapter.md) |
