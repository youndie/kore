...
| `kore-core/src/commonMain/kotlin/io/github/youndie/kore/runtime/` | **built (B-52)** — `containerMemoryBudget()`, the three answers, and the cgroup walk behind one file-reading function so every branch is a common test. `applyHeapCeiling()` is the lever and nothing here calls it: `GC.maxHeapBytes`, not `targetHeapBytes`, which autotune rewrites — measured |
